...
int main()
{
	...
	while (true)
	{
		...
		for (UsbDeviceDesc it : deviceArray)
			cout << deviceId++ << ". " << it.friendlyName << (it.removable ? "\t\tREMOVABLE" : "\t\tNOT REMOVABLE") << endl;
		...
		if (!choice + '0' == 'u')
			continue;
		if (static_cast<int>(choice) > deviceArray.size())
			continue;

		if (deviceArray[choice - 1].removable)
		{
			CM_Request_Device_EjectW(deviceArray[choice - 1].devInst, NULL, NULL, NULL, NULL);
			system("pause");
		}
		else
		{
			cout << "Device " << static_cast<int>(choice) << " is NOT removable!\n";
			system("pause");
		}
	}
